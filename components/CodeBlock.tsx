...
const Line: FC<LineProps> = ({ lineNumber, children, showLineNumbers }) => (
  <span className="flex">
    {showLineNumbers && (
      <span className="linenumber inline-block min-w-[2.75em] select-none pr-[1em] text-right text-indigo-200">
        {lineNumber}
      </span>
    )}
    {children}
...
const CodeBlock: FC<CodeBlockProps> = ({
      ...
      if (
        ...
        if (foundSkipLine) {
          ...
          return (
            <span
              key={i}
              className="bg-secondary my-1 flex items-center justify-center whitespace-pre rounded-sm py-1 text-xs"
            >
              {`... omitted ${startSkip - removedCount}-${endSkip - removedCount} ...`}
            </span>
          )
        } else {
          return null
        ...
        if (
          ...
          lineStyle.backgroundColor = 'rgba(112, 255, 148, 0.1)'
          // lineStyle.width = '100%'
        } else if (
          removedLines.some(
  ...
  return (
    <div
      ...
      onMouseLeave={onExit}
      className="border-border relative my-6 overflow-hidden rounded-lg border"
      ref={textInput}
    >
      {loading ? (
        // Loading circle using Tailwind
        <div className="flex h-40 items-center justify-center">
          <div className="border-border h-12 w-12 animate-spin rounded-full border-t-2"></div>
        </div>
      ) : (
        <>
          {fileName || title || id ? (
